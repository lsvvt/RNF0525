...
def get_mo_coeff(id, mol, xc):
    if "piM06" in xc:
        dirf = f"orca_HOCl_wfn/orca_HOCl/PBE-2X/"
    elif "NN-PBE" in xc:
        dirf = f"orca_HOCl_wfn/orca_HOCl/PBE-2X/"
    else:
        dirf = f"orca_HOCl_wfn/orca_HOCl/{xc}/"
    for f in os.listdir(dirf):
        if id == f[:-41]:
            hf_wfn = dirf + f + "/input.wfn"

    mo_coeff, mo_occ, mo_energy, dm = tutil.readwfn(hf_wfn, mol, makerdm=True)
    return dm
...
def compute_energy_pyscf(geometry_data, basis_set, xc, fyaml):
    ...
    if xc == "DM21":
        mf.xc = 'B3LYP'
        dm0 = get_mo_coeff(geometry_data["id"], mol, xc)
        ...
        if fyaml == "h4_database.yaml":
            ...
            # dm0 = get_mo_coeff(geometry_data["id"], mol)
            # mf.DIIS = scf.ADIIS
        ...
        dm = get_mo_coeff(geometry_data["id"], mol, xc)
        if len(dm_l) > 0:
...
def main(basis_set, xc, fyaml):
    ...
    for rxn in reactions:
        ...
        # print(E_reactants_hartree, E_products_hartree)
...
if __name__ == "__main__":
    ...
    # for i, xc in enumerate(table(["DM21", "NN-PBE", "LDA", "M06-L", "PBE", "TPSS", "r2SCAN", "SCAN", "SCAN0", "PBE-2X", "PBE0", "M06-2X", "M05-2X", "B3LYP", "DM21", "piM06-2X-DL", "piM06-2X"])):
    # for i, xc in enumerate(table(["DM21", "PBE0", "LDA", "PBE-2X", "M06-L", "PBE", "TPSS", "r2SCAN", "SCAN", "SCAN0"])):

    for i, xc in enumerate(table(["NN-PBE", "M06-2X", "DM21", "PBE0", "PBE-2X", "PBE", "LDA", "M06-L", "TPSS", "r2SCAN", "SCAN", "SCAN0", "M05-2X", "B3LYP", "piM06-2X-DL", "piM06-2X"])):
