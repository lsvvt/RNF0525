...
def get_match(wfn, ref):
    ...
    for b in wfn:
        ...
        for n, k in enumerate(ref):
            if b[0] == k[0] and b[1][0] == k[1]:
                d_tmp = abs(b[2] - float(k[2]))
                if d_tmp < d:
                    a = n
                    d = d_tmp
...
def readwfn(filename, mol, makerdm=False):
    ...
    with open(filename, 'r') as f:
        ...
        coord = []
        for i in range(natm):
            dat = f.readline().split("(CENTRE")[1].replace("-", " -").split()
            achrg.append(float(dat[-1]))
            coord.append([float(c) for c in dat[1:4]])
        ...
        eng = []
        for n in range(norb):
            dat = f.readline().replace("ORB. ENERGY =", "ORB. ENERGY = ").split()
    ...
    # float_formatter = truncate
    uncmol_info = []
    for ib in range(uncmol.nbas):
        ia = uncmol.bas_atom(ib)
        l = uncmol.bas_angular(ib)
        es = uncmol.bas_exp(ib)
        uncmol_info.append([ia + 1, l, float(es[0])])

    match = []
    match2 = []
    for ip in range(nprm):
        am = [l for l in range(6) if typea[ip] in TYPE_MAP[l]]
        #        print(expos[ip], uncmol.bas_exp(ip))
        #        match.append(uncmol_info.index([centr[ip], am, float(expos[ip])]))
        match2.append([centr[ip], am, expos[ip]])

    match = get_match(match2, uncmol_info)
    ...
    def eval_vhm(C, ZvH, d_atom, rad, lmax, n_coords, n_rad):
        ...
        def _convert_coeff(c, x):
            ...
            for i in range(nx - 1):
                ...
                nc[3, :] = c[3, :] - 1 * c[2, :] * x[:nx - 1] + 1 * c[1, :] * x[:nx - 1] ** 2 - 1 * c[0, :] * x[
                                                                                                              :nx - 1] ** 3
            return nc
