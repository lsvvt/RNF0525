...
for i in range(11):  # Generate 11 geometries (i from 0 to 10)
    coord_val.append(0.45 + i / 20.0)

data = {"r":[], "xc": [], "basis": [], "e": []}

for f in os.listdir("bak"):
    if "H4" in f:
        with open("bak/" + f, "r") as fil:
            d = f.split("_")
            data["e"].append(float(fil.read()))
            data["r"].append(coord_val[int(d[2])])
            data["basis"].append(d[-1])
            data["xc"].append(d[3])
...
names = ["FCI / SUMR-CCSD", "CCSD", "PBE_ref", "B3LYP_ref", "O3P86"]
symbols = ["square-x", "square-open", "triangle-up-open", "circle-open", "circle"]

for i in range(int(len(y) / len(x))):
    data["e"] += y[i * len(x):(i + 1) * len(x)][::-1]
    data["r"] += list(np.array(x[::-1]) / 2)
    data["basis"] += len(x) * ["6-31G"]
    data["xc"] += [names[i]] * len(x)
...
for f in datain:
    ...
    if name == "DM21":
        ...
        data["e"] += list(y)[1:]
        data["r"] += list(np.array(x[::-1]) / 2)[1:]
        data["basis"] += (len(x) - 1) * ["6-31G"]
        data["xc"] += ["DM21"] * (len(x) - 1)
...
df.to_csv('h4_new_data.csv', index=False)  